...
function fundAll() {
  exe(`${cli} keys generate ${process.env.SOROBAN_ACCOUNT} --network testnet`);
}
...
function deploy(wasm) {
  exe(`${cli} contract deploy --wasm ${wasm} --ignore-checks --alias ${filenameNoExtension(wasm)} --network testnet`);
}
...
function bind({alias, id}) {
  exe(`${cli} contract bindings typescript --contract-id ${id} --network testnet --output-dir ${dirname}/packages/${alias} --overwrite`);
}
